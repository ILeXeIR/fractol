...
/*   Updated: 2024/01/26 20:38:34 by alpetukh      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static int32_t	color_schema_1(uint32_t iteration, uint32_t max_iter)
{
	int32_t	r;
	int32_t	g;
	int32_t	b;
...
static int32_t	color_schema_2(uint32_t iteration, uint32_t max_iter)
{
	if (iteration == max_iter)
		return (0xFF);
...
static int32_t	color_schema_3(uint32_t iteration, uint32_t max_iter)
{
	int32_t	r;
	int32_t	g;
	int32_t	b;
	...
	color = color_schema_1(iteration, max_iter);
	r = (color >> 24);
	b = 255 - ((color >> 16) & 0xFF);
	g = (color >> 8) & 0xFF;
	a = color & 0xFF;
...
{
	if (color_schema == 1)
		return (color_schema_1(iteration, max_iter));
	else if (color_schema == 2)
		return (color_schema_2(iteration, max_iter));
	else if (color_schema == 3)
		return (color_schema_3(iteration, max_iter));
	return (0);
}
